...
function onlyEvenValues(arr){
    // function to return boolean: true if even, false if odd
    filterEvens = function(num) {
        if (num % 2 === 0) {
            return true;
        }
    }
    return arr.filter(filterEvens);
}
...
function showFirstAndLast(arr){
    ...
    firstLast = function(inputString) {
...
function vowelCount(str){
    // initialize object with vowels as keys and counts as 0
...
function doubleValuesWithMap(arr) {
    mappedArr = arr.map(function(num) {
        return num * 2;
    });
...
function filterByValue(arr, key) {
    filtered = arr.filter(function(obj) {
        return key in obj;
    });
