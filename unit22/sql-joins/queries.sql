...
select first_name, last_name, count(owner_id) as count_of_vehicles
from owners
...
select first_name, last_name, avg(price) as avg_car_price, count(owner_id) as count_of_vehicles
from owners
    join vehicles on owners.id = vehicles.owner_id
group by first_name, last_name
